...
class TestCreateFilesystemsCommand(TestCase, metaclass=PatchTrackedRemoteExecution):
    ...
    def test_execute(self):
        self._init_test_data('ubuntu16', 'target__filesystem_creation')

        CreateFilesystemsCommand(self.source).execute()

        self.assertIn('sudo mkfs.ext4 -u 549c8755-2757-446e-8c78-f76b50491f21 /dev/vdb1', self.executed_commands)
        self.assertIn('sudo mkfs.ext3 -u d04ba532-cd2d-4406-a5ef-114acf019cc8 /dev/vdc', self.executed_commands)
        self.assertIn('sudo mkfs.ext4 -u 53ad2170-488d-481a-a6ab-5ce0e538f247 /dev/vdd1', self.executed_commands)
        self.assertIn('sudo mkfs.ext4 -u bcab224c-8407-4783-8cea-f9ea4be3fabf /dev/vdd2', self.executed_commands)

    def test_execute__non_supported_filesystem(self):
        self._init_test_data('ubuntu16__lvm', 'target__filesystem_creation')
